...
def accept_client(server):
    while True:
        ...
        client_thread = Thread(target=handle_client, args=(client_socket, ), daemon=True)
        client_thread.start()
...
def make_file(path:str, client_socket:socket, open_mode: str="wb"):
    with open(path, open_mode) as f, client_socket.makefile("rb", buffering=74806245) as sf:
...
def main():
    ...
    server.bind(ADDR); print(f"Server spinning up at {ADDR}")
    server.listen(5); print(f"Listening for connections...")
